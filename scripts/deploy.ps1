...
param (
    ...
    [string]$Platform = "x64",
    [string]$ServiceName = "ominull"
)

$ErrorActionPreference = "Stop"

Write-Host "=== Deploying ominull to target $TargetHost ===" -ForegroundColor Cyan
...
Copy-Item -Path $sysPath -Destination "$targetDrv\ominull.sys" -Force
Copy-Item -Path $cerPath -Destination "$targetDrv\ominull.cer" -Force
Write-Host "Files copied to $targetDrv" -ForegroundColor Green

Write-Host "`nInstructions on Target VM:" -ForegroundColor Yellow
Write-Host "1. Import Cert: certutil -addstore -f Root C:\drv\ominull.cer"
Write-Host "                certutil -addstore -f TrustedPublisher C:\drv\ominull.cer"
Write-Host "2. Create Service: sc.exe create $ServiceName type= kernel binPath= C:\drv\ominull.sys"
