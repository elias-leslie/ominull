...
(async()=>{
 ...
 while(Date.now()-start<30*60*1000){
  ...
  fs.mkdirSync('build',{recursive:true});fs.writeFileSync('build/console-soak.json',JSON.stringify({running:true,installed_app:true,samples,errors},null,2));
  await new Promise(r=>setTimeout(r,15000));
 }
 const longTasks=await page.evaluate(()=>window.soakTasks);
 const result={browser:browser.version(),viewport:[1280,720],cpu_slowdown:4,network:{latency_ms:150,download_bytes_per_second:187500},duration_ms:Date.now()-start,installed_app:true,physical_battery_measured:false,errors,longTasks,samples};
