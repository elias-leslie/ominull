...
6. **Nothing From The Network Reaches A Shell:** Values that fan out to agents — a quarantined peer's address, an isolation allow list — are validated as addresses at the hub and again at the agent, and are applied through an explicit argument vector rather than a constructed command line.
7. **Zero WFP Object Leaks:** Driver unload routines dynamically unregister all callouts, sublayers, and ALE filter rules, guaranteeing clean state restoration on service teardown.
