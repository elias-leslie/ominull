  ...
  <form id="setup-form">
    ...
    <section class="setup-step" data-step="1" hidden>
      ...
      <div class="setup-fields">
        ...
        <label class="field-wide">Console canonical hostname (FQDN)<input name="console_hostname" placeholder="hub.lan or omi.example.invalid" required><span class="why">Required for browser WebCrypto and WebAuthn passkeys (cannot be an IP address). <strong>Warning: changing this hostname later invalidates all enrolled WebAuthn passkeys.</strong></span></label>
        <label class="field-wide">Console URL<input name="console_url" type="url" required placeholder="https://hub.lan:8443"></label>
        <label class="field-wide">HTTPS agent URL<input name="agent_url" type="url" required placeholder="https://hub.lan:9443"></label>
      </div>
      <p class="pending">LAN console uses dedicated HTTPS; agent traffic uses mTLS. WAN modes use separate HTTPS console and agent hostnames. Keep interactive sign-in off agent hostname.</p>
    ...
    <section class="setup-step" data-step="2" hidden>
      <h2>Certificates and sign-in</h2><p>Choose console certificate source: hub-issued CA with guided root trust, automated ACME via DNS-01, or operator-supplied certificate.</p>
      <div class="setup-fields">
        <label>TLS certificate source<select name="tls_mode"><option value="self-issued">Hub-issued CA with guided browser root trust (LAN / internal)</option><option value="acme">Automated Let's Encrypt ACME (DNS-01)</option><option value="custom">Operator-supplied certificate & key pair</option></select></label>
        ...
        <div class="setup-group setup-fields" id="hub-ca-info">
          <div class="field-wide"><h3>Hub internal CA & browser trust</h3><p class="why">The hub issues a dedicated leaf certificate from its internal CA. To establish a secure context for WebCrypto/WebAuthn in browsers on this network, export the root CA using <code>ominullctl console export-ca</code> and install it in your operating system / browser trust store. <a class="btn btn-sm" href="/api/v1/pki/ca.crt" download="ca.crt">Download Root CA</a></p></div>
        </div>
        ...
        <div class="setup-group setup-fields" id="acme-fields">
          <div class="field-wide"><h3>Automated ACME (DNS-01)</h3><p class="why">Obtains a trusted Let's Encrypt certificate using DNS-01 challenge. Works behind NAT without inbound port exposure.</p></div>
          <label>ACME contact email<input name="acme_email" type="email" placeholder="ops@example.invalid"></label>
          <label>DNS-01 provider<select name="acme_dns_provider"><option value="cloudflare">Cloudflare DNS API</option><option value="manual">Manual DNS TXT records</option></select></label>
        </div>
...
` + diagnosticsRendererJS + `
  ...
  q("#setup-form").addEventListener("submit",async function(event){event.preventDefault();if(!event.target.reportValidity())return;var form=new FormData(event.target),save=q("#save-message");message(save,"Saving validated configuration…","");try{var body={configuration:{network_mode:form.get("network"),console_hostname:form.get("console_hostname"),console_url:form.get("console_url"),agent_url:form.get("agent_url"),tls_mode:form.get("tls_mode"),client_certs:form.get("client_certs"),tls_cert_file:form.get("tls_cert_file"),tls_key_file:form.get("tls_key_file"),tls_hosts:String(form.get("tls_hosts")||"").split(",").map(function(v){return v.trim();}).filter(Boolean),acme_enabled:form.get("tls_mode")==="acme",acme_email:form.get("acme_email"),acme_dns_provider:form.get("acme_dns_provider"),oidc_issuer:form.get("oidc_issuer"),oidc_client_id:form.get("oidc_client_id"),oidc_redirect_url:form.get("oidc_redirect_url"),access_team:form.get("access_team"),access_audience:form.get("access_audience"),cloudflare:form.get("network")==="cloudflare"},local_admin_email:form.get("email"),oidc_client_secret:form.get("oidc_client_secret")};var result=await api("/api/v1/setup/apply",{method:"POST",body:JSON.stringify(body)});message(save,result.restart_required?"Saved. Restart ominull-hub.service, create a fresh setup token, then return here to install and verify.":"Configuration saved.",result.restart_required?"warn":"ok");if(!result.restart_required){await load();show(3);}}catch(error){message(save,error.message,"crit");}});
