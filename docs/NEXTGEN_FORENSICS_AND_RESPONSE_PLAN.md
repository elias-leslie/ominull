...
| Phase 5 | Slices 5A & 5B Verified; Slices 5C-5D in progress | Slice 5A on 2026-09-04 (`docs/evidence/5A-1.md`) hardened versioned script library with typed parameter schemas, strict interpreter allowlisting (`/bin/sh`, `/bin/bash`, `powershell.exe`, `cmd.exe`, `pwsh.exe`), tenant isolation across all queries/mutations, fail-closed console execution gating (static API keys return 403 Forbidden), independent SHA-256 digest recomputation, and full CLI parity in `ominullctl scripts` (`list`, `show`, `create`, `update`, `retire`). Slice 5B on 2026-09-04 (`docs/evidence/5B-1.md`) implemented the pure-C Linux script execution worker in `agent/include/script_exec_linux.h` and `agent/linux/main.c` with process group containment (`setpgid`, descriptor closure, `clearenv()`, `RLIMIT_CPU`, timeout escalation), in-process SHA-256 digest verification, parameter delivery via private file and environment variables without template substitution, and bounded output capture. Windows endpoint worker (5C) and Web Console sheet (5D) in progress. |
...
- **Slice 5C (Windows Endpoint Script Execution Worker & Job Object Containment)**: In progress.
  Implement Windows script execution worker with Windows Job Object containment (`JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE`, suspended process launch, environment sanitization), PowerShell/cmd parameter delivery, bounded execution, and exit reporting.
- **Slice 5D (Web Console Script Library UI & Execution Sheets)**: Backlog.
