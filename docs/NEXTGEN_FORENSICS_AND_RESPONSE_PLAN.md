...
|---|---|---|
| Phase 0 | Slice 0.1 implemented; baseline and threat model pending | Slice 0.1 on 2026-09-03 (`docs/evidence/0-1.md`) consolidated fixtures into `hub/tests/fixtures/response`, deleted duplicate `tests/fixtures/`, added 21 comprehensive cross-language fixtures (missing optional/required, unknown fields, max size, cryptographic tampering, old-agent behavior), and verified consumption in Go and C. Slices 0.2 (canonical byte encoder), 0.3 (threat model), and 0.4 (baseline measurements) remain to be completed for full Phase 0 acceptance. |
