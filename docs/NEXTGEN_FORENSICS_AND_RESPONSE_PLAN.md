...
| Phase 3 | Slices 3.0a–3A.1 verified; slices 3A.2–3D.1 pending | Slices 3.0a–3.0d (`docs/evidence/3-0a.md`, `docs/evidence/3-0b.md`, `docs/evidence/3-0c.md`, `docs/evidence/3-0d.md`) implemented dedicated console TLS, multi-source certificates, real browser WebAuthn verification, and reverse proxy specifications. Slice 3A.1 on 2026-09-03 (`docs/evidence/3A-1.md`) implemented the full-duplex authenticated WebSocket relay (`hub/pkg/terminal/relay.go`) with token authentication, HttpOnly cookie issuance, zero-exposure token guarantees, 1 MiB bounded buffer flow control, 25s ping/pong keepalives, and verified bidirectional `stdin`/`stdout`/`resize` frame relay and clean closure handling with race detection. |
