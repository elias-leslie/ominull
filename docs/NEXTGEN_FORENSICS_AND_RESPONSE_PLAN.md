...
| Phase 3 | No usable shell; false console claims removed | Corrected by slice R0.4 on 2026-09-03 (`docs/evidence/R0-4.md`): the console shell modal, context menu action, and shell launch card have been removed so the UI makes no false claims of active, encrypted, relayed, or audited pseudoterminals. Backend routes fail closed under R0.2. There is currently no WebSocket route, PTY, ConPTY, pairing, or byte relay. |
