...
|---|---|---|
| Phase 0 | Slices 0.1 and 0.2 implemented; threat model and baseline pending | Slice 0.1 on 2026-09-03 (`docs/evidence/0-1.md`) consolidated fixtures into `hub/tests/fixtures/response`, deleted duplicate `tests/fixtures/`, added 21 comprehensive cross-language fixtures (missing optional/required, unknown fields, max size, cryptographic tampering, old-agent behavior), and verified consumption in Go and C. Slice 0.2 on 2026-09-03 (`docs/evidence/0-2.md`) replaced colon-joined formatting with a deterministic length-prefixed binary canonical encoder in Go (`hub/pkg/response/canonical.go`) and C (`agent/include/response_canonical.h`), bumped `GrantVersion` and `ProofVersion` to 2, and proved bit-for-bit identity against byte-exact binary fixtures. Slices 0.3 (threat model) and 0.4 (baseline measurements) remain to be completed for full Phase 0 acceptance. |
