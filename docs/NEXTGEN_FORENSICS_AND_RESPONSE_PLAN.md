...
| Phase 1B | Slices 1B.1, 1B.2 verified; slices 1B.3-1B.4 in progress | Slice 1B.1 on 2026-09-03 (`docs/evidence/1B-1.md`) replaced in-memory maps with durable SQLite signer store (`hub/pkg/responseauth/store.go`). Slice 1B.2 on 2026-09-03 (`docs/evidence/1B-2.md`) hardened TOTP with AES-256-GCM secret encryption at rest, 5-attempt lockout, single-use timestep anti-replay, root recovery token consumption, and honest phishing-vulnerability security labeling. Slices 1B.3 (WebAuthn) and 1B.4 (grant validation) underway. |
