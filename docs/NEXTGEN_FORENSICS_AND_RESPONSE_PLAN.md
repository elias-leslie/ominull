...
| Phase 5 | Slice 5A Verified; Slices 5B-5D in progress | Slice 5A on 2026-09-04 (`docs/evidence/5A-1.md`) hardened versioned script library with typed parameter schemas, strict interpreter allowlisting (`/bin/sh`, `/bin/bash`, `powershell.exe`, `cmd.exe`, `pwsh.exe`), tenant isolation across all queries/mutations, fail-closed console execution gating (static API keys return 403 Forbidden), independent SHA-256 digest recomputation, and full CLI parity in `ominullctl scripts` (`list`, `show`, `create`, `update`, `retire`). Endpoint process containment (5B, 5C) and Web Console sheet (5D) in progress. |
...
- **Slice 5B (Linux Endpoint Script Execution Worker & Process Containment)**: In progress.
  Implement Linux script execution worker in `agent/include/script_exec_linux.h` and hook into `agent/linux/main.c`. Enforce process containment (`setpgid`, `close(fd > 2)`, `clearenv()`, `RLIMIT_CPU`, execution timeout), deliver parameters via private file or protected pipe without textual template substitution, capture bounded stdout/stderr, and report exit status.
- **Slice 5C (Windows Endpoint Script Execution Worker & Job Object Containment)**: Backlog.
