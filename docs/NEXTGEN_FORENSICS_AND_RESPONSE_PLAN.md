...
| Phase 6 | Slice 6A Verified; Slices 6B–6D in progress | Slice 6A on 2026-09-04 (`docs/evidence/6A-1.md`) implemented the additive SQLite schema, automatic migrations, telemetry ingestion pipeline, and query projections for process lineage (`process_instance_id`, `parent_pid`, `parent_process_instance_id`, `command_line`, `user_identity`, `attribution_status`, `observed_at`) and executable hashing (`executable_sha256`) across `hub/pkg/storage` and `hub/pkg/server` with full backward/forward compatibility and API parity. Linux agent worker (6B) in progress. |
...
- **Slice 6B (Linux Endpoint Process Lineage, Boot Identity, & Executable Hashing)**: In progress.
  Linux daemon (`ominulld`) boot ID capture (`/proc/sys/kernel/random/boot_id`), bounded `/proc` sampling (`/proc/<pid>/stat`, `status`, `cmdline`), user-space LRU executable hash cache keyed on `dev+ino+size+mtime` with size caps (16 MiB max file, byte limits), safe `O_NOFOLLOW` file opening, attribution status handling (`authoritative`, `inferred_cached`, `race_suspected`, `permission_denied`, `unknown`), and flow telemetry batch enrichment.
