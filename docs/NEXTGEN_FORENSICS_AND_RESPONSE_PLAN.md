...
| Phase 4 | In progress (Slices 4A.1, 4A.2, 4B complete) | Slices 4A.1 on 2026-09-04 (`docs/evidence/4A-1.md`), 4A.2 on 2026-09-04 (`docs/evidence/4A-2.md`), and 4B on 2026-09-04 (`docs/evidence/4B-1.md`) implemented allowlisted diagnostic and live volatile profile forensic collections on Linux (`agent/include/forensics_linux.h`) and Windows (`agent/include/forensics_windows.h`). Live volatile collection implements 6 allowlisted collectors on Linux (procfs process snapshot, socket-to-process correlation, utmp sessions, arp neighbors, iptables firewall state, loaded modules) and 6 on Windows (Toolhelp32/PSAPI process snapshot, extended TCP/UDP socket-to-process, WTS sessions, GetIpNetTable neighbors, Defender firewall registry state, loaded device drivers). Both platforms enforce honest status attribution, 512 KiB item and 10 MiB bundle bounds, thread worker isolation, dynamic profile dispatching, in-process TweetNaCl detached Ed25519 canonical binary manifest V2 signing (`OMINULL-MANIFEST-V2\0`), persistent endpoint evidence keys, hub fail-closed response gates, auto-provisioned evidence bundles, WinHTTP/libcurl chunked uploads, and signed hub receipt chaining. Verified under pure-C tests, Go race detector (`TestServer_LiveVolatileForensicFlow`), and live package lifecycle rehearsals. Slice 4C remains. |
...
- **Slice 4C (IR Standard Profile Forensic Collection & Console Integration)**: Allowlisted persistence,
  scheduled tasks, event log extraction, console inspection and export flows.
