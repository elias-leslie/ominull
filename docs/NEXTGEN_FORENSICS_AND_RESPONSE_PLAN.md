...
| Phase 1B | Verified (Slices 1B.1, 1B.2, 1B.3, 1B.4 complete) | Slice 1B.1 (`docs/evidence/1B-1.md`) implemented durable SQLite signer store with restart persistence. Slice 1B.2 (`docs/evidence/1B-2.md`) hardened TOTP with secret encryption, lockout, and anti-replay. Slice 1B.3 (`docs/evidence/1B-3.md`) implemented pure-Go WebAuthn with trusted origin, user presence, and anti-cloning. Slice 1B.4 on 2026-09-03 (`docs/evidence/1B-4.md`) implemented typed action validation, independent digest recomputation, strict proof binding (tenant/kind/targets), bounded target sets, and length-prefixed canonical Ed25519 grant signing. Ready for operator acceptance. |
| Phase 1C | Verified (Slices 1C.1, 1C.2 complete) | Slice 1C.1 (`docs/evidence/1C-1.md`) implemented state transition validation (`IsValidTransition`), tenant and endpoint binding on ACK/progress/result/cancel, replay protection, and `response_job_audit` migration. Slice 1C.2 on 2026-09-03 (`docs/evidence/1C-2.md`) implemented independent server-side SHA-256 digest recomputation on every response route (`/api/v1/response/jobs`, `/api/v1/scripts/run`, `/api/v1/terminal/sessions`) and ingress identity-header stripping (`X-Operator-ID`). Ready for operator acceptance. |
| Phase 1D | Verified (Slices 1D.1, 1D.2 complete) | Slice 1D.1 on 2026-09-03 (`docs/evidence/1D-1.md`) implemented the Linux response dispatcher in `agent/include/response_dispatcher.h` and `agent/linux/main.c`, including bounded offer parser, FIPS 180-4 SHA-256 digest recomputation, canonical Ed25519 `EndpointGrant` V2 verification, durable locked replay cache (`replay_cache.state`), strict acknowledgment gating, and child worker containment. Slice 1D.2 on 2026-09-03 (`docs/evidence/1D-2.md`) implemented the Windows response dispatcher in `agent/src/response_windows.c`, `hub_client.c`, and `service.c`, including in-process pure-C RFC 8032 Ed25519 verification (`agent/include/ed25519_verify.h`), Win32 durable locked replay cache (`LockFileEx`/`FlushFileBuffers`), Windows Job Object containment (`JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE`, suspended process creation, sanitized environment, kill-on-cancel), and WinHTTP authenticated response transport (`Hub_PostPathJSON`). Ready for operator acceptance. |
| Phase 1E | Verified (Slice 1E.1 complete) | Slice 1E.1 on 2026-09-03 (`docs/evidence/1E-1.md`) completed the native Go CLI refactor in `hub/cmd/ominullctl/main.go`, achieving full command parity with `scripts/ominull-cli`, porting status, scanner, alerts, mesh quarantine, agent updates, and bootstrap reports. Preserved setup-token handling, enforced API key file ingestion with zero plaintext environment fallback, implemented response job inspection/cancellation, response-auth status and recovery tokens, enforced fail-closed rejection of console-only actions (`shell open`, `scripts run`, `forensics launch`), added comprehensive unit tests in `main_test.go`, updated `README.md` and `PLAN.md`, and deleted `scripts/ominull-cli`. Ready for operator acceptance. |
| Phase 2 | Verified (Slices 2.1–2.5 complete) | Slices 2.1–2.5 (`docs/evidence/2-1.md`, `docs/evidence/2-2.md`, `docs/evidence/2-3.md`, `docs/evidence/2-4.md`, `docs/evidence/2-5.md`) implemented bounded resumable chunked upload with 8MB `MaxBytesReader`, range overlap detection in `evidence_chunks`, tenant quota and filesystem headroom checks, atomic writes with `O_NOFOLLOW` / `fsync` / rename, length-prefixed binary AEAD associated data, secret sanitization, tenant and device endpoint scoping on all routes, endpoint manifest Ed25519 signing and verification, hub receipt Ed25519 signing and chaining, safe path layout with manifest and receipt bundling in archives, streaming export limits, honest export error auditing, offline verification via `ominullctl forensics verify`, and retention pruning engine with legal hold immunity and actor/reason attribution. Ready for operator acceptance. |
| Phase 3 | Verified (Slices 3.0a–3D.1 complete) | Slices 3.0a–3.0d (`docs/evidence/3-0a.md`, `docs/evidence/3-0b.md`, `docs/evidence/3-0c.md`, `docs/evidence/3-0d.md`) implemented dedicated console TLS, multi-source certificates, real browser WebAuthn verification, and reverse proxy specifications. Slice 3A.1 on 2026-09-03 (`docs/evidence/3A-1.md`) implemented the full-duplex authenticated WebSocket relay (`hub/pkg/terminal/relay.go`). Slice 3A.2 on 2026-09-03 (`docs/evidence/3A-2.md`) implemented durable SQLite session state, caps, sweeper, and token hashing. Slice 3B.1 on 2026-09-03 (`docs/evidence/3B-1.md`) implemented the Linux `forkpty` pseudoterminal worker with strict program allowlist (`/bin/sh`, `/bin/bash`), environment sanitization, dynamic `TIOCSWINSZ` resize propagation, fail-closed child-tree process termination (`SIGKILL` to process group), and authenticated loopback relay via libcurl over WebSocket. Slice 3B.2 on 2026-09-03 (`docs/evidence/3B-2.md`) implemented the Windows ConPTY worker with fixed executable choices (`cmd.exe`, `powershell.exe`, `pwsh.exe`), Windows Job Object containment (`JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE`), environment sanitization, dynamic resize, native WinHTTP WebSocket relay, raised Windows compilation baseline to `NTDDI_VERSION 0x0A000006` and `_WIN32_WINNT 0x0A00`, and documented the Windows 10 1809 / Server 2019 OS floor. Slice 3C.1 on 2026-09-03 (`docs/evidence/3C-1.md`) implemented console response unlock, controller window, volatile in-memory WebCrypto Ed25519 browser key generation, typed ActionProof V2 signing, and terminal session dispatch. Slice 3C.2 on 2026-09-03 (`docs/evidence/3C-2.md`) implemented vendored xterm.js (zero CDN), honest session UI, live limit indicators, and interactive controls verified in Chrome 150 against live packaged deployment. Slice 3D.1 on 2026-09-04 (`docs/evidence/3D-1.md`) implemented bounded timestamped frame recording (`stdin`, `stdout`, `resize`, `close`), 10 MiB cap to bounded marker, encrypted evidence store bundle sealing (`session_frames.ndjson`, AES-256-GCM, signed receipt), and fail-closed static API key and external frame injection route gates. |
| Phase 4 | Verified (Slices 4A.1, 4A.2, 4B, 4C complete) | Slices 4A.1 on 2026-09-04 (`docs/evidence/4A-1.md`), 4A.2 on 2026-09-04 (`docs/evidence/4A-2.md`), 4B on 2026-09-04 (`docs/evidence/4B-1.md`), and 4C on 2026-09-04 (`docs/evidence/4C-1.md`) implemented complete diagnostic (8 artifacts), live volatile (14 artifacts), and IR standard (18 artifacts) forensic collections on Linux and Windows. Collectors enforce honest status attribution, 512 KiB item and 10 MiB bundle caps, thread/process containment, TweetNaCl detached Ed25519 canonical manifest V2 signing (`OMINULL-MANIFEST-V2\0`), persistent endpoint evidence keys, hub fail-closed gates, automatic bundle provisioning, and signed receipt chaining. Implemented hub bundle items query API (`GET /api/v1/evidence/items?bundle_id=`) and web console integration (target picker, forensic collection dispatch sheet with WebCrypto Ed25519 ActionProof V2 signing, bundle inspection modal, and legal hold compliance toggle). Verified under pure-C tests on Linux and Windows (Wine), Go race detector, package lifecycle rehearsals, and live headless Google Chrome 150 execution on LXC 150 and live endpoint. Phase 4 fully complete. |
| Phase 5 | Verified (Slices 5A–5D complete) | Slice 5A on 2026-09-04 (`docs/evidence/5A-1.md`) hardened versioned script library with typed parameter schemas, strict interpreter allowlisting (`/bin/sh`, `/bin/bash`, `powershell.exe`, `cmd.exe`, `pwsh.exe`), tenant isolation across all queries/mutations, fail-closed console execution gating (static API keys return 403 Forbidden), independent SHA-256 digest recomputation, and full CLI parity in `ominullctl scripts` (`list`, `show`, `create`, `update`, `retire`). Slice 5B on 2026-09-04 (`docs/evidence/5B-1.md`) implemented the pure-C Linux script execution worker in `agent/include/script_exec_linux.h` and `agent/linux/main.c` with process group containment (`setpgid`, descriptor closure, `clearenv()`, `RLIMIT_CPU`, timeout escalation), in-process SHA-256 digest verification, parameter delivery via private file and environment variables without template substitution, and bounded output capture. Slice 5C on 2026-09-04 (`docs/evidence/5C-1.md`) implemented the pure-C Windows script execution worker in `agent/include/script_exec_windows.h` and `agent/src/response_windows.c` with Windows Job Object containment (`JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE`), suspended process creation, pipe output redirection, NUL stdin handle, in-process SHA-256 digest verification, parameter delivery via private file and sanitized environment block, and millisecond timeout monitoring (`TerminateJobObject(124)`). Slice 5D on 2026-09-04 (`docs/evidence/5D-1.md`) implemented the Web Console script library UI, dynamic schema form, WebCrypto Ed25519 signing, single job output inspection modal, and schedule freezing. Phase 5 fully complete. |
| Phase 6 | Verified (Slices 6A–6D complete) | Slice 6A on 2026-09-04 (`docs/evidence/6A-1.md`) implemented additive SQLite schema, automatic migrations, telemetry ingestion pipeline, and query projections for process lineage and executable hashing across `hub/pkg/storage` and `hub/pkg/server` with full backward/forward compatibility and API parity. Slice 6B on 2026-09-04 (`docs/evidence/6B-1.md`) implemented Linux endpoint daemon (`ominulld`) boot identity capture, bounded `/proc` sampling (`stat`, `status`, `cmdline`), 512-entry user-space LRU executable hash cache (`dev+ino+size+mtime`), size bounding (64 MiB), attribution statuses (`authoritative`, `inferred_cached`, `race_suspected`, `permission_denied`, `unknown`), zero-invention parent instance handling, telemetry batch serialization, pure-C unit test suite under `-Wall -Wextra -Werror -O2`, and live container deployment on LXC 140/150 with bit-for-bit SHA-256 parity and REST API verification. Slice 6C on 2026-09-04 (`docs/evidence/6C-1.md`) implemented Windows endpoint daemon (`ominulld.exe`) boot identity binding, dynamic `ntdll.dll` lookup for parent PID and command line, user token resolution, 512-entry Win32 LRU executable hash cache, ETW process provider integration with snapshot fallback, verified via 46 pure-C tests under Wine. Slice 6D on 2026-09-04 (`docs/evidence/6D-1.md`) implemented Web Console process lineage hierarchy tree visualization, attribution badges, SHA-256 badges, command-line inspector, forensics sheet modal, fleet filter bar query chips, and Asset view recent flows launcher, verified via headless Chrome 150 CDP rehearsal against live deployment. Phase 6 fully complete. |
| Phase 7 | Verified (Slices 7A–7D complete) | Slice 7A on 2026-09-04 (`docs/evidence/7A-1.md`) implemented authoritative installed software inventory collectors on Linux and Windows, source-aware deduplication (`idx_software_identity`), raw vs normalized field preservation, scope/confidence fields, additive SQLite schema migrations, and paginated tenant-scoped APIs. Slice 7B on 2026-09-04 (`docs/evidence/7B-1.md`) implemented the versioned vulnerability feed ingestion engine and atomic snapshot lifecycle across NVD 2.0, CISA KEV, and EPSS (`vuln_feed_snapshots`, `snapshot_vulnerabilities`, `snapshot_cisa_kev`), HTTP 429 rate limit and corruption isolation, reproducible snapshot queries, and REST APIs. Slice 7C on 2026-09-04 (`docs/evidence/7C-1.md`) implemented the authoritative CPE 2.3 parser (`hub/pkg/vuln/cpe.go`), Debian/RPM/SemVer/Windows version comparison engine (`hub/pkg/vuln/version.go`), range boundary evaluation (`[start, end)`), candidate classification (`matched`, `not_affected`, `possible`, `insufficient_data`), independent source value preservation, transparent prioritization scoring (0.0–100.0), and explainable evidence generation. Slice 7D on 2026-09-04 (`docs/evidence/7D-1.md`) implemented Web Console software inventory inspection, active feed snapshot lifecycle strip, interactive feed sync modal, explainable vulnerability correlation table, audit evidence modal sheet (`.sheet-wide`), and automated headless Google Chrome 150 CDP rehearsal against live LXC 150. Phase 7 fully complete. |
...
| 2026-09-03 | Decided in Slice 0.3. Windows agent OS floor for interactive shell is Windows 10 Version 1809 (Build 17763) / Windows Server 2019 (`NTDDI_VERSION 0x0A000006`). Pre-1809 builds support telemetry and forensic collection with shells disabled. See `docs/RESPONSE_THREAT_MODEL_AND_TRUSTED_ORIGIN.md`. |
...
| 1A.1 | `verified` | `docs/evidence/1A-1.md` | Migrated hub process to unprivileged `ominull:ominull` account; updated service unit and postinst; verified live telemetry, package serving, certificate handling, diagnostics, and quarantine on LXC 150. |
| 1A.2 | `verified` | `docs/evidence/1A-2.md` | Packaged response authority under dedicated `ominull-authority:ominull-authority` identity; systemd unit `response-authority.service`; restricted control socket 0660; verified mutual isolation and package lifecycle. |
...
| 1B.2 | `verified` | `docs/evidence/1B-2.md` | Hardened TOTP with AES-256-GCM secret encryption at rest, 5-attempt lockout, single-use timestep anti-replay, root recovery token consumption, and honest phishing-vulnerability security labeling; verified on live LXC 150. |
| 1B.3 | `verified` | `docs/evidence/1B-3.md` | Pure-Go WebAuthn manager in `hub/pkg/responseauth/webauthn.go`; ES256/Ed25519/RS256 support; trusted-origin validation, user presence enforcement, sign counter anti-cloning, and phishing-resistant labeling; verified on live LXC 150. |
| 1B.4 | `verified` | `docs/evidence/1B-4.md` | Typed action validation with independent SHA-256 digest recomputation; proof binding to tenant, action kind, and target endpoint list; unbounded target set denial; single-use proof nonce replay protection; length-prefixed canonical grant signing (`OMINULL-ENDPOINT-GRANT-V2`). |
...
| Slice | Status | Evidence | Description |
|---|---|---|---|
| 1C.1 | `verified` | `docs/evidence/1C-1.md` | State transition validation (`IsValidTransition`), tenant and endpoint binding across all job operations, lease expiry and idempotency replay protection, incremental progress tracking, and durable relational audit logging in `response_job_audit`. Verified on live LXC 150. |
| 1C.2 | `verified` | `docs/evidence/1C-2.md` | Independent server-side SHA-256 digest recomputation on all response routes; client digest and proof digest mismatch rejection; ingress `X-Operator-ID` stripping in `authMiddleware` and `device_auth`. Verified on live LXC 150. |
...
| Slice | Status | Evidence | Description |
|---|---|---|---|
| 1D.1 | `verified` | `docs/evidence/1D-1.md` | Linux response dispatcher with bounded offer parser (max 4 offers, 64KB payload), in-process SHA-256 digest recomputation, canonical Ed25519 EndpointGrant V2 verification, durable locked replay cache (`replay_cache.state`), strict acknowledgment gating, and contained child execution. Tested with `-Wall -Wextra -Wformat=2 -O2` and verified on live endpoint. |
| 1D.2 | `verified` | `docs/evidence/1D-2.md` | Windows response dispatcher with bounded offer parser (max 4 offers, 64KB payload), in-process pure-C Ed25519 verification (`agent/include/ed25519_verify.h`), Win32 locked replay cache (`C:\ProgramData\Ominull\replay_cache.state`), strict WinHTTP acknowledgment gating (`Hub_PostPathJSON`), and Windows Job Object containment (`JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE`, suspended process execution, clean envBlock, kill-on-cancel). Tested with MinGW GCC under `-Wall -Wextra -Wformat=2 -O2` and verified under Wine. |
...
| Slice | Status | Evidence | Description |
|---|---|---|---|
| 1E.1 | `verified` | `docs/evidence/1E-1.md` | Native Go CLI (`ominullctl`) refactored into subcommands with full command and alias parity for all retained `scripts/ominull-cli` operations. Added endpoint, response-job, response-auth status, local recovery token rotation, structured `--json` formatting, fail-closed blocking of console-only actions, and comprehensive unit tests. `scripts/ominull-cli` safely deleted. |
...
| Slice | Status | Evidence | Description |
|---|---|---|---|
| 2.1 | `verified` | `docs/evidence/2-1.md` | Bounded resumable chunked upload with 8MB `MaxBytesReader`, range overlap detection in `evidence_chunks`, tenant quota and filesystem headroom validation, atomic no-follow writes with fsync and rename, length-prefixed AEAD associated data, secret sanitization, and strict tenant scoping. |
| 2.2 | `verified` | `docs/evidence/2-2.md` | Strict tenant scoping across every evidence store method and HTTP handler, device credential support for evidence/response machine routes, and device endpoint binding preventing cross-endpoint upload or finalization. |
| 2.3 | `verified` | `docs/evidence/2-3.md` | Endpoint manifest Ed25519 signing and verification, dedicated hub receipt Ed25519 signing, unambiguous length-prefixed canonical serialization, and tamper-evident chaining. |
| 2.4 | `verified` | `docs/evidence/2-4.md` | Safe sanitized archive layout (`bundleID/safeName`) bundling manifest.json and receipt.json, streaming export quota limits, honest failure auditing (`EVIDENCE_EXPORT_FAILED`), and offline CLI verification in `ominullctl forensics verify`. |
| 2.5 | `verified` | `docs/evidence/2-5.md` | Retention pruning engine with disk cleanup of unreferenced objects, legal hold immunity preserving expired bundles, actor and reason attribution on hold changes, and `ominullctl forensics prune` integration. |
...
- **Slice 4A.1 (Linux Diagnostic Profile Forensic Collection)**: Verified (`docs/evidence/4A-1.md`).
  Implemented 8 allowlisted Linux diagnostic collectors (`os_version.json`, `network_interfaces.json`,
  `routes.txt`, `dns_config.txt`, `resource_summary.json`, `service_state.json`, `system_logs.txt`,
  `agent_diagnostics.json`), bounded collection sizes (512 KiB per item, 10 MiB bundle), contained
  child worker isolation (`setpgid`, `close(fd > 2)`, `clearenv`, safe cwd, `RLIMIT_CPU`), in-process
  pure-C TweetNaCl detached Ed25519 signing (`agent/include/ed25519_verify.h`), persistent endpoint
  evidence key (`/var/lib/ominull/evidence_signer.key`), canonical binary manifest V2
  (`OMINULL-MANIFEST-V2\0`), hub fail-closed static API key gate on `POST /api/v1/response/jobs`,
  automatic evidence bundle provisioning, and live verification with signed hub receipts on LXC 150.
- **Slice 4A.2 (Windows Diagnostic Profile Forensic Collection)**: Verified (`docs/evidence/4A-2.md`).
  Implemented 8 allowlisted native Win32 diagnostic collectors (`os_version.json`, `network_interfaces.json`,
  `routes.txt`, `dns_config.txt`, `resource_summary.json`, `service_state.json`, `system_logs.txt`,
  `agent_diagnostics.json`), bounded collection sizes (512 KiB per item, 10 MiB bundle), Win32 thread worker
  isolation, BCrypt RNG-seeded in-process TweetNaCl detached Ed25519 signing (`agent/include/ed25519_verify.h`),
  persistent endpoint evidence key (`C:\ProgramData\Ominull\evidence_signer.key`), canonical binary manifest V2
  (`OMINULL-MANIFEST-V2\0`), WinHTTP chunked evidence upload via `Hub_PostPathData`, and signed hub receipt verification.
  Verified with pure-C tests under Wine, Go Hub race tests, and cross-compilation package builds.
- **Slice 4B (Live Volatile Profile Forensic Collection)**: Verified on 2026-09-04 (`docs/evidence/4B-1.md`).
  Implemented 6 allowlisted native Linux and Windows live volatile collectors (`process_snapshot.json`,
  `socket_to_process.json`, `logged_in_sessions.json`, `network_neighbors.json`, `firewall_state.json`,
  `loaded_modules.json`), dynamic profile dispatching (`Forensics_RunCollection` / `Forensics_RunCollectionWin`),
  unified bundle finalization (`Forensics_PublishBundleAndFinalize`), 512 KiB item caps, in-process TweetNaCl
  detached Ed25519 canonical binary manifest V2 signing (`OMINULL-MANIFEST-V2\0`), persistent endpoint evidence keys,
  and end-to-end Go Hub integration testing (`TestServer_LiveVolatileForensicFlow`) verifying manifest verification,
  receipt signing, and full tar.gz archive export.
- **Slice 4C (IR Standard Profile Forensic Collection & Console Integration)**: Verified on 2026-09-04 (`docs/evidence/4C-1.md`).
  Implemented 4 allowlisted incident response collectors on Linux and Windows (`persistence.json`,
  `scheduled_tasks.json`, `security_events.txt`, `shell_history.txt`) bringing the `ir_standard` bundle
  to 18 total artifacts. Added hub evidence items query API (`GET /api/v1/evidence/items?bundle_id=`)
  with `GetBundleItems`. Integrated Web Console with target endpoint picker (`openLaunchForensicsPicker`),
  forensic collection dispatch sheet (`launchForensicsSheet`) with profile selection, bounds, timeout, and
  WebCrypto Ed25519 ActionProof V2 signing, bundle inspector modal (`inspectBundle`) rendering bundle summary
  and all 18 artifact rows with status badges and SHA-256 digests, and legal hold compliance controls (`toggleBundleHold`).
  Verified via pure-C tests on Linux and Windows (Wine), Go Hub integration tests under `-race`, native package builds,
  and live headless Google Chrome 150 verification on LXC 150 (`10.0.0.58`) and live Linux endpoint (`10.0.0.57`).
...
- **Slice 5A (Versioned Script Library Hardening, Parameter Schema Validation, & CLI Parity)**: Verified (`docs/evidence/5A-1.md`).
  Implemented strongly-typed parameter schema validation (`ValidateSchema`, `ValidateParameters`), strict interpreter allowlisting (`/bin/sh`, `/bin/bash`, `powershell.exe`, `cmd.exe`, `pwsh.exe`), tenant isolation across all script queries and mutations, independent SHA-256 payload digest recomputation, execution bounds clamping, fail-closed static API key gating on `POST /api/v1/scripts/run` (HTTP 403 Forbidden; requires active console response session with WebCrypto Ed25519 ActionProof V2), and native CLI commands in `ominullctl scripts` (`list`, `show`, `create`, `update`, `retire`, rejection of `run`/`schedule`). Verified under Go race detector, package builds, and Debian lifecycle rehearsals.
- **Slice 5B (Linux Endpoint Script Execution Worker & Process Containment)**: Verified (`docs/evidence/5B-1.md`).
  Implemented pure-C Linux script execution worker in `agent/include/script_exec_linux.h` and integrated into daemon response offer dispatcher (`agent/linux/main.c`). Enforces process containment (`setpgid(0, 0)`, descriptor closure for `fd > 2`, `clearenv()`, `RLIMIT_CPU`, millisecond timeout escalation from `SIGTERM` to `SIGKILL`), in-process SHA-256 digest recomputation, parameter delivery via private file (`/var/lib/ominull/ominull_params_<job_id>.json`, mode `0600`) and environment variables (`OMINULL_PARAM_<NAME>`) without textual template substitution, bounded combined stdout/stderr capture (capped at 5 MiB with truncation notice), and result reporting. Verified via pure-C unit test suite `agent/tests/test_script_exec_linux.c` (11 tests, 27 assertions, 0 failures), full package builds, and Debian lifecycle rehearsals.
- **Slice 5C (Windows Endpoint Script Execution Worker & Job Object Containment)**: Verified (`docs/evidence/5C-1.md`).
  Implemented pure-C Windows script execution worker in `agent/include/script_exec_windows.h` and integrated into daemon response dispatcher (`agent/src/response_windows.c`). Enforces strict interpreter allowlisting (`cmd.exe`, `cmd`, `powershell.exe`, `powershell`, `pwsh.exe`, `pwsh`), Windows Job Object containment (`JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE | JOB_OBJECT_LIMIT_DIE_ON_UNHANDLED_EXCEPTION`), suspended process creation, anonymous pipe output redirection, `NUL` standard input handle, in-process SHA-256 digest recomputation (`Response_SHA256_Sum`), parameter delivery via private file (`C:\ProgramData\Ominull\ominull_params_<job_id>.json`) and sanitized environment block (`OMINULL_PARAM_<NAME>`, `<NAME>`) without textual template substitution, millisecond timeout monitoring (`TerminateJobObject(124)`), and output capture bounds (up to 5 MiB cap). Verified via pure-C unit test suite `agent/tests/test_script_exec_windows.c` under Wine (73 tests, 0 failures), full binary builds, package builds, and Debian lifecycle rehearsals.
- **Slice 5D (Web Console Script Library UI & Execution Sheets)**: Verified (`docs/evidence/5D-1.md`).
  Implemented Web Console script library UI sheet (`hub/pkg/server/web/app.js`, `app.css`), dynamic parameter input form generation from schema (supporting enum, boolean, number, string, ipv4, cidr), WebCrypto Ed25519 ActionProof V2 signing requesting canonical server digest (`POST /api/v1/scripts/digest`), script run output inspection modal with live stdout/stderr viewing (`GET /api/v1/response/jobs?id=`), and schedule freezing to immutable script digest and explicit target endpoints snapshot (`GET|POST|DELETE /api/v1/scripts/schedules`). Verified via Go unit tests under race detector, package builds, and live headless Google Chrome 150 CDP rehearsal against live hub on LXC 150 and live Linux endpoint on LXC 140. Phase 5 is 100% complete.
...
- **Slice 6A (Process Lineage & Executable Hashing Database Schema, Migrations, Ingestion, & Query Parity)**: Verified (`docs/evidence/6A-1.md`).
  Implemented additive SQLite schema with 8 enrichment columns (`process_instance_id`, `parent_pid`, `parent_process_instance_id`, `command_line`, `user_identity`, `executable_sha256`, `attribution_status`, `observed_at`), automatic startup migrations with zero data loss, batch ingestion in `telemetry_batch.go`, storage queries with `COALESCE` / `sql.NullTime`, traffic flow projections in `traffic.go`, and API parity across `/api/v1/events` and `/api/v1/traffic/flows`. Verified via unit test suites `lineage_test.go` and `lineage_api_test.go` under Go race detector, full binary builds, package builds, and Debian lifecycle rehearsals.
- **Slice 6B (Linux Endpoint Process Lineage, Boot Identity, & Executable Hashing)**: Verified (`docs/evidence/6B-1.md`).
  Implemented Linux daemon (`ominulld`) boot ID capture (`/proc/sys/kernel/random/boot_id` with caching and fallback), bounded `/proc` sampling (`/proc/<pid>/stat`, `status`, `cmdline`), 512-entry user-space LRU executable hash cache keyed on `dev+ino+size+mtime` with 64 MiB ceiling (`LINUX_MAX_HASH_BYTES`), attribution status handling (`authoritative`, `inferred_cached`, `race_suspected`, `permission_denied`, `unknown`), zero-invention parent instance ID handling on parent termination, zero-allocation JSON escaping, flow telemetry batch enrichment, pure-C unit test suite `test_process_lineage_linux.c` passing under `-Wall -Wextra -Werror -O2`, native Debian packaging, and live deployment on LXC 140/150 with bit-for-bit SHA-256 verification and REST API query parity.
- **Slice 6C (Windows Endpoint Process Lineage, ETW Process Provider, & Executable Hashing)**: Verified (`docs/evidence/6C-1.md`).
  Implemented Windows agent daemon (`ominulld.exe`) boot identity binding (`win-%08lx-%llu` with deterministic volume serial and boot tick subtraction), process creation time via `GetProcessTimes` in 100ns ticks, parent PID and UTF-8 command line resolution via dynamic `ntdll.dll` lookup of `NtQueryInformationProcess` (`ProcessBasicInformation` and `ProcessCommandLineInformation = 60`) with snapshot fallback, user token identity resolution via `OpenProcessToken` and `LookupAccountSidA`, 512-entry user-space LRU executable hash cache keyed on `(volume_serial, file_index, file_size, last_write_time)` from `BY_HANDLE_FILE_INFORMATION` with 64 MiB ceiling, real-time ETW Process Provider initialization (`Microsoft-Windows-Kernel-Process` GUID `{22fb2cd6-0e7b-4226-a0f7-2d4c022a0044}`) with graceful snapshot fallback, attribution status semantics (`authoritative`, `inferred_cached`, `race_suspected`, `permission_denied`, `unknown`), zero-allocation JSON escaping, flow telemetry batch enrichment in `agent/src/service.c` with intra-batch PID deduplication and serialization in `agent/src/hub_client.c`. Verified via pure-C unit test suite `test_process_lineage_windows.c` under Wine (46 assertions, 0 failures), full native binary builds, Windows MSI packaging, Debian package lifecycle verification, and end-to-end baseline measurement.
- **Slice 6D (Web Console Process Lineage Tree, Executable Hash Telemetry, & Fleet Investigation UI)**: Verified (`docs/evidence/6D-1.md`).
  Implemented Web Console process lineage hierarchy tree visualization (`.process-lineage-tree` with parent PPID, parent instance ID, branch connector `↳`, and child PID/instance ID), attribution status badges with distinct confidence styles (`authoritative`, `inferred_cached`, `race_suspected`, `permission_denied`, `unknown`), executable SHA-256 badge with one-click copy and fleet-wide search filtering, monospace command-line execution inspector with scroll and copy utility, full process forensics & event inspection modal sheet (`.sheet-forensics`), fleet filter bar query chips (`tf.hash`, `tf.user`, `tf.attribution`), and Asset view "Recent flows" forensics launcher. Verified via unit tests, package lifecycle verification, and live automated Google Chrome 150 CDP rehearsal against the live Hub and Linux endpoint container. Phase 6 is 100% complete.
...
- **Slice 7A Status**: Verified (`docs/evidence/7A-1.md`). Authoritative installed package collection implemented for Linux (`/var/lib/dpkg/status` parsing with installed status filter and RPM query fallback in `agent/include/software_inventory_linux.h`) and Windows (`Uninstall` registry key enumeration across 64-bit and WOW6432Node in `agent/include/software_inventory_windows.h`). Preserves raw values beside normalized values. Additive SQLite migrations (`install_scope`, `confidence`, `raw_vendor`, `raw_product`, `raw_version`) and unique index `idx_software_identity`. Pure-C unit test suites passed on Linux (46 tests, 2,954 host packages parsed) and Windows (31 tests under Wine), and Go tests passed under `-race`. Packaging and baseline checks passed.
...
- **Slice 7B Status**: Verified (`docs/evidence/7B-1.md`). Versioned feed snapshots and atomic activation lifecycle implemented (`vuln_feed_snapshots`, `snapshot_vulnerabilities`, `snapshot_cisa_kev` in `hub/pkg/vuln/snapshot.go`). Parsers and streaming network fetchers implemented for NVD 2.0 (`ParseNVD20`, `ParseNVD20WithTotal`, `FetchNVD20Page`), CISA KEV (`ParseCISAKEV`, `FetchCISAKEV`), and FIRST EPSS (`ParseEPSSJSON`, `FetchEPSS`). Atomic promotion preserves active snapshot when incoming feeds are interrupted, corrupted, or rate-limited (HTTP 429). Verified reproducible snapshot queries, pagination, and REST APIs (`POST /api/v1/vulnerabilities/sync`, `GET /api/v1/vulnerabilities/snapshots`, `GET /api/v1/vulnerabilities?scope=catalog`). Verified via Go unit and server tests under race detection with zero regressions.
...
- **Slice 7C Status**: Verified (`docs/evidence/7C-1.md`). CPE 2.3 formatted string and URI parser implemented (`hub/pkg/vuln/cpe.go`). Debian epoch/revision, SemVer prerelease, and Windows dotted version comparison engine implemented (`hub/pkg/vuln/version.go`). Range boundary evaluation (`versionStartIncluding/Excluding`, `versionEndIncluding/Excluding`) implemented, distinguishing `matched`, `not_affected`, `possible`, and `insufficient_data`. Explainable structured evidence (`VulnerabilityMatchEvidence`) serialized per match. Independent source preservation (`cvss`, `is_kev`, `epss`, `severity`, `confidence`) and transparent prioritization scoring (0.0–100.0) implemented and verified across official conformance boundary cases (OpenSSH CVE-2024-6387, Log4j CVE-2021-44228, XZ Utils CVE-2024-3094, Sudo CVE-2021-3156), full Go race detector runs, Debian/Windows packaging, and baseline measurements.

### 7D. Web Console UI & CDP rehearsal

- Display installed software inventory in endpoint/asset route drawer with live filtering, package manager badges (`dpkg`, `rpm`, `win_registry`), architecture, install scope, and authoritative/inferred confidence badges.
- Display active vulnerability feed snapshot metadata strip on Forensics view (`snapshot_id`, NVD CVE count, CISA KEV count, EPSS prediction count, activation timestamp) and interactive sync modal (`POST /api/v1/vulnerabilities/sync`).
- Display explainable vulnerability correlation table with CVSS severity badges, composite priority scores (0.0–100.0), CISA KEV exploitation badges, EPSS prediction badges, and verdict badges (`Affected`, `Possible`, `Not Affected`, `Inconclusive`).
- Explainable audit evidence sheet (`.sheet-wide`) displaying detected product/version, raw installed package string, source, target CPE criteria pattern, vulnerable version range boundaries, natural language comparison rationale, and raw structured JSON audit trace.
- **Slice 7D Status**: Verified (`docs/evidence/7D-1.md`). Automated headless Google Chrome 150 CDP rehearsal (`scratch/verify_vuln_inventory_ui.mjs`) verified against live LXC 150 (`ominull-hub`). Verified snapshot strip, feed sync modal, correlation table, explainable audit evidence modal sheet with raw JSON trace, and asset route installed software inventory with real-time filter search. Phase 7 100% complete.
...
| 3.0a | `verified` | `docs/evidence/3-0a.md` | Dedicated console HTTPS listener in the hub (`--console-tls-listen`), separate from the agent `:9443` listener, with `ClientAuth: tls.NoClientCert` and HSTS header injection. |
| 3.0b | `verified` | `docs/evidence/3-0b.md` | Console certificate sources: operator-supplied, ACME DNS-01, and hub-CA leaf with guided root trust; installer asks for the console hostname. |
| 3.0c | `verified` | `docs/evidence/3-0c.md` | Verify secure context, WebCrypto, and WebAuthn in a real browser against the packaged install on the hub host. |
| 3.0d | `verified` | `docs/evidence/3-0d.md` | Remaining Phase 3.0 prerequisites and the documented reverse-proxy shape. |
| 3A.1 | `verified` | `docs/evidence/3A-1.md` | WSS transport spike: agent to hub to console, authenticated loopback. |
| 3A.2 | `verified` | `docs/evidence/3A-2.md` | Durable SQLite session state machine, limits (1 per endpoint, 4 per tenant), background sweeper with connect/idle/duration timeouts, daemon restart recovery, and hashed one-use relay tokens. |
| 3B.1 | `verified` | `docs/evidence/3B-1.md` | Linux `forkpty` worker with fixed program choices (`/bin/sh`, `/bin/bash`), sanitized environment, dynamic `TIOCSWINSZ` resize propagation, and fail-closed child-tree process termination (`SIGKILL` to process group). |
| 3B.2 | `verified` | `docs/evidence/3B-2.md` | Windows ConPTY worker in a Job Object; raise the Windows build baseline to `NTDDI_VERSION 0x0A000006` and record the new OS floor. |
| 3C.1 | `verified` | `docs/evidence/3C-1.md` | Console response unlock, controller window, volatile in-memory WebCrypto Ed25519 browser key, typed ActionProof V2 signing, and terminal session dispatch. |
| 3C.2 | `verified` | `docs/evidence/3C-2.md` | Vendored terminal emulator (xterm.js/fit addon, zero CDN) and honest session UI (`sheet-terminal`) with live metadata, untrusted text handling, and interactive controls. |
| 3D.1 | `verified` | `docs/evidence/3D-1.md` | Frame recording through the evidence store: exact timestamped frames (`stdin`, `stdout`, `resize`, `close`), 10 MiB byte bounding to marker frame, sealing to encrypted evidence bundle (`session_frames.ndjson`, AES-256-GCM, signed receipt), fail-closed route gates rejecting static API keys and external frame injection. |
