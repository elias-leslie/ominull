...
| Phase 2 | Slices 2.1, 2.2 verified; slices 2.3–2.5 pending | Slice 2.1 on 2026-09-03 (`docs/evidence/2-1.md`) implemented bounded resumable chunked upload with `MaxBytesReader` (8MB cap), range tracking (`evidence_chunks`), overlap rejection (`ErrRangeOverlap`), tenant quota and free disk headroom checks, atomic writes with `O_NOFOLLOW` / `fsync` / rename, length-prefixed AEAD associated data, secret sanitization, and strict tenant scoping. Slice 2.2 on 2026-09-03 (`docs/evidence/2-2.md`) enforced tenant scoping across all store methods and handlers, updated `deviceOrLegacyMiddleware` to allow permitted device routes, and bound device credentials to their enrolled `EndpointID` preventing cross-endpoint item upload or bundle finalization. |
