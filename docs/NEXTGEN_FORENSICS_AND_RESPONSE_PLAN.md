...
- **Slice 6C (Windows Endpoint Process Lineage, ETW Process Provider, & Executable Hashing)**: Planned.
  Windows agent daemon (`ominulld.exe`) real-time ETW process provider, process instance ID generation, Win32 user-space hash cache (volume serial + file index + mtime), command line capture, user token attribution, and flow event enrichment.
