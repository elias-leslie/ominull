...
| Phase 1D | Slice 1D.1 verified; slice 1D.2 in progress | Slice 1D.1 on 2026-09-03 (`docs/evidence/1D-1.md`) implemented the Linux response dispatcher in `agent/include/response_dispatcher.h` and `agent/linux/main.c`, including bounded offer parser over a fixed schema, in-process FIPS 180-4 SHA-256 digest recomputation, canonical Ed25519 `EndpointGrant` V2 cryptographic verification, durable locked replay cache (`replay_cache.state`), strict acknowledgment gating, and child worker containment (process groups, closed descriptors, wiped environment, resource limits). Slice 1D.2 (Windows grant verifier and worker containment) underway. |
