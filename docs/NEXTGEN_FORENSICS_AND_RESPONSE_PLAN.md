...
| Phase 7 | Slices 7A & 7B Verified; Slices 7C–7D in progress | Slice 7A on 2026-09-04 (`docs/evidence/7A-1.md`) implemented authoritative installed software inventory collectors on Linux and Windows, source-aware deduplication (`idx_software_identity`), raw vs normalized field preservation, scope/confidence fields, additive SQLite schema migrations, and paginated tenant-scoped APIs. Slice 7B on 2026-09-04 (`docs/evidence/7B-1.md`) implemented the versioned vulnerability feed ingestion engine and atomic snapshot lifecycle across NVD 2.0, CISA KEV, and EPSS (`vuln_feed_snapshots`, `snapshot_vulnerabilities`, `snapshot_cisa_kev`), HTTP 429 rate limit and corruption isolation (staging failures leave active snapshot untouched), reproducible snapshot queries, and REST APIs (`/api/v1/vulnerabilities/sync`, `/api/v1/vulnerabilities/snapshots`, `/api/v1/vulnerabilities?scope=catalog`). Verified via Go race test suites across all 21 hub packages. CPE version range matching (7C) and Web Console UI (7D) in progress. |
