...
| Phase 3 | Slices 3.0a, 3.0b verified; slices 3.0c–3D.1 pending | Slice 3.0a on 2026-09-03 (`docs/evidence/3-0a.md`) implemented the dedicated console HTTPS listener (`--console-tls-listen`) separated from the `:9443` agent transport listener. Slice 3.0b on 2026-09-03 (`docs/evidence/3-0b.md`) implemented the three console TLS certificate sources (operator-supplied, automated ACME DNS-01 with Cloudflare API v4, and internal hub-CA leaf with guided root trust), installer hostname capture with WebAuthn RP ID invariant enforcement (strictly rejecting IP addresses), and `ominullctl console export-ca` / `trust-instructions` / `status` tooling. |
