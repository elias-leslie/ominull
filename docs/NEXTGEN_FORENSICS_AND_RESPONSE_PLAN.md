...
| Phase 4 | In progress (Slice 4A.1 complete) | Slice 4A.1 on 2026-09-04 (`docs/evidence/4A-1.md`) implemented the Linux diagnostic profile forensic collection (`agent/include/forensics_linux.h`), including 8 allowlisted bounded collectors with honest status reporting, contained child worker execution (`setpgid`, `close(fd > 2)`, `clearenv`, safe cwd, `RLIMIT_CPU`), in-process TweetNaCl detached Ed25519 manifest signing (`agent/include/ed25519_verify.h`), persistent endpoint evidence key (`/var/lib/ominull/evidence_signer.key`), canonical binary manifest V2 (`OMINULL-MANIFEST-V2\0`), hub fail-closed static API key gate on `POST /api/v1/response/jobs`, automatic evidence bundle provisioning, and live verification with signed hub receipts on LXC 150. Windows and additional profiles remain to be completed. |
...
- **Slice 4A.2 (Windows Diagnostic Profile Forensic Collection)**: Windows allowlisted
  diagnostic collectors, Job Object process containment, WinHTTP evidence uploads, and manifest signing.
