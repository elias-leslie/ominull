...
| Phase 1B | Slices 1B.1, 1B.2, 1B.3 verified; slice 1B.4 in progress | Slice 1B.1 on 2026-09-03 (`docs/evidence/1B-1.md`) replaced in-memory maps with durable SQLite signer store (`hub/pkg/responseauth/store.go`). Slice 1B.2 on 2026-09-03 (`docs/evidence/1B-2.md`) hardened TOTP with AES-256-GCM secret encryption at rest, 5-attempt lockout, and anti-replay. Slice 1B.3 on 2026-09-03 (`docs/evidence/1B-3.md`) implemented WebAuthn registration and authentication (ES256, Ed25519, RS256) with trusted-origin enforcement, user presence verification, sign-counter anti-cloning, and phishing-resistant security labeling. Slice 1B.4 (grant validation) underway. |
