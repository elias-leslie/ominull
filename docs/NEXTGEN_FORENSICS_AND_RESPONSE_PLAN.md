...
| Phase 3 | Slices 3.0a–3B.1 verified; slices 3B.2–3D.1 pending | Slices 3.0a–3.0d (`docs/evidence/3-0a.md`, `docs/evidence/3-0b.md`, `docs/evidence/3-0c.md`, `docs/evidence/3-0d.md`) implemented dedicated console TLS, multi-source certificates, real browser WebAuthn verification, and reverse proxy specifications. Slice 3A.1 on 2026-09-03 (`docs/evidence/3A-1.md`) implemented the full-duplex authenticated WebSocket relay (`hub/pkg/terminal/relay.go`). Slice 3A.2 on 2026-09-03 (`docs/evidence/3A-2.md`) implemented durable SQLite session state, caps, sweeper, and token hashing. Slice 3B.1 on 2026-09-03 (`docs/evidence/3B-1.md`) implemented the Linux `forkpty` pseudoterminal worker with strict program allowlist (`/bin/sh`, `/bin/bash`), environment sanitization, dynamic `TIOCSWINSZ` resize propagation, fail-closed child-tree process termination (`SIGKILL` to process group), and authenticated loopback relay via libcurl over WebSocket. |
