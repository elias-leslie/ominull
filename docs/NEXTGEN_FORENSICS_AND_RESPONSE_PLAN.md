...
| Phase 3 | Slices 3.0a, 3.0b, 3.0c verified; slices 3.0d–3D.1 pending | Slice 3.0a on 2026-09-03 (`docs/evidence/3-0a.md`) implemented dedicated console HTTPS listener (`--console-tls-listen`). Slice 3.0b on 2026-09-03 (`docs/evidence/3-0b.md`) implemented the three console TLS certificate sources, WebAuthn RP ID invariant enforcement, and `ominullctl console` export/status tools. Slice 3.0c on 2026-09-03 (`docs/evidence/3-0c.md`) deployed the package to LXC 150 under hypervisor snapshot protection, exported and trusted the hub root CA, and verified secure context (`window.isSecureContext === true`), WebCrypto (ECDSA P-256 key generation, signing, and verification), WebAuthn passkey registration, and RP ID IP-rejection invariant in real headless Google Chrome on a remote origin. |
