...
| Phase 3 | Slices 3.0a–3A.2 verified; slices 3B.1–3D.1 pending | Slices 3.0a–3.0d (`docs/evidence/3-0a.md`, `docs/evidence/3-0b.md`, `docs/evidence/3-0c.md`, `docs/evidence/3-0d.md`) implemented dedicated console TLS, multi-source certificates, real browser WebAuthn verification, and reverse proxy specifications. Slice 3A.1 on 2026-09-03 (`docs/evidence/3A-1.md`) implemented the full-duplex authenticated WebSocket relay (`hub/pkg/terminal/relay.go`). Slice 3A.2 on 2026-09-03 (`docs/evidence/3A-2.md`) implemented durable SQLite session state, composite indexes, 1-per-endpoint and 4-per-tenant concurrency caps, automated background sweeping (30s connect timeout, idle timeout, max duration), daemon restart recovery, and strict token privacy (SHA-256 `TokenHash` storage, zero plaintext leakage in DB/JSON/summaries). |
