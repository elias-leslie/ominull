...
| Phase 4 | In progress (Slices 4A.1, 4A.2 complete) | Slices 4A.1 on 2026-09-04 (`docs/evidence/4A-1.md`) and 4A.2 on 2026-09-04 (`docs/evidence/4A-2.md`) implemented allowlisted diagnostic profile forensic collections on Linux (`agent/include/forensics_linux.h`) and Windows (`agent/include/forensics_windows.h`). Both platforms enforce honest status attribution, 512 KiB item and 10 MiB bundle bounds, contained worker execution, in-process TweetNaCl detached Ed25519 canonical binary manifest V2 signing (`OMINULL-MANIFEST-V2\0`), persistent endpoint evidence keys, hub fail-closed response gates (reject static keys on `POST /api/v1/response/jobs` with 403), auto-provisioned evidence bundles, WinHTTP/libcurl chunked uploads, and signed hub receipt chaining. Verified under pure-C tests, race detector, and live package lifecycle rehearsals. Slices 4B and 4C remain. |
...
- **Slice 4B (Live Volatile Profile Forensic Collection)**: Linux and Windows live volatile collectors
  (process snapshots, socket-to-process, sessions, neighbors, firewall, loaded modules).
