...
| Phase 7 | Slice 7A Verified; Slices 7B–7D in progress | Slice 7A on 2026-09-04 (`docs/evidence/7A-1.md`) implemented authoritative installed software inventory collectors on Linux (parsing `/var/lib/dpkg/status` for installed status and RPM query fallback) and Windows (enumerating 64-bit and WOW6432Node Uninstall registry keys), source-aware deduplication (`idx_software_identity`), raw vs normalized field preservation, scope/confidence fields, additive SQLite schema migrations, and paginated tenant-scoped APIs. Verified via pure-C tests on Linux (46 assertions, live 2,954 host packages parsed) and Windows under Wine (31 assertions), Go race tests, package lifecycle verification, and baseline runs. Feed ingestion (7B), CPE matching (7C), and Web Console UI (7D) in progress. |
