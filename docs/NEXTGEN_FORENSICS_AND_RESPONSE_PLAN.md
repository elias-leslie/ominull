...
- **Slice 6D (Web Console Process Lineage Tree, Executable Hash Telemetry, & Fleet Investigation UI)**: Planned.
  Web Console event inspection sheet, process hierarchy tree visualization (Parent -> Child PID), executable SHA-256 hash badge with one-click copy and search, attribution status badges, monospace command line inspector, and live Google Chrome 150 CDP rehearsal.
